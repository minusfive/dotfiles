...
function {
  ...
  mv ~/Desktop/Screenshot* "$__screenshots_dir" 2>/dev/null || true
  ...
  # Menu bar
  log_info "Set $(underline menu bar) to show date and time"
  defaults write com.apple.menuextra.clock DateFormat -string "\"EEE d MMM HH:mm\""

  # Spaces
  log_info "Set $(underline Dock) to use separate spaces on each display"
  ...
  if [[ $? == 0 ]]; then
    ...
    # Menu bar
    log_ok "$(underline menu bar) set to show date and time"

    # Spaces
    log_ok "$(underline Dock) set to use separate spaces on each display"

    # Restart SystemUIServer to apply screenshots changes
  ...
  log_info "Set $(underline Finder) to show folders first"
  defaults write com.apple.finder "_FXSortFoldersFirst" -bool true

  log_info "Set $(underline Finder) to open folders in tabs on command + double-click"
  defaults write com.apple.finder "FinderSpawnTab" -bool true

  log_info "Set $(underline Finder) default search scope to current folder"
  ...
  # Desktop
  log_info "Set $(underline Finder) to sort folders first on desktop"
  ...
  if [[ $? == 0 ]]; then
    ...
    log_ok "$(underline Finder) set to show folders first"
    log_ok "$(underline Finder) set to open folders in tabs on command + double-click"
    log_ok "$(underline Finder) default search scope set to current folder"
    ...
    # Desktop
    log_ok "$(underline Finder) set to show folders first on desktop"
  ...
  # Spaces
  log_info "Set $(underline Dock) to switch spaces on app activation"
  ...
  if [[ $? == 0 ]]; then
    ...
    log_ok "$(underline Dock) set to switch spaces on app activation"
