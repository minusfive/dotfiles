...
local function killHelper() hs.alert.closeSpecific(previousHelperID) end
...
function obj.recursiveBind(keymap)
  ...
  for key, map in pairs(keymap) do
    ...
    if #key >= 3 then keyFuncNameTable[createKeyName(key)] = key[3] end
  end
  return function()
    modal:enter()
    killHelper()
    if obj.showBindHelper then showHelper(keyFuncNameTable) end
  end
end
